(function (window, document, jQuery, undefined) {
	...
	function page() {
		...
		this._defaultUnit   = $('.social-wall').data('default-unit');
		this._masonryArray  = [];
	}

	projects.$w.load(function(){
		// 取出一半的社群內容做為第二次瀑布流用
		if (pageObj._masonryLength > pageObj._defaultUnit) {
			for (var i = pageObj._masonryLength - 1; i >= pageObj._defaultUnit; i--) {
				var $this = $('.social-wall .list-item').eq(i);

				if ($this[0] !== undefined) {
					pageObj._masonryArray.push($this[0].outerHTML);
					$('.social-wall .list-item').eq(i).remove();
				}
			}
