(function (window, document, jQuery, undefined) {
    ...
    page.prototype.Angular = function(element, callback) {
        var _elem = typeof(element) === 'object' ? jQuery(element) : element;
    ...
    page.prototype.AngularDate = function() {
        ...
        var $tags = jQuery(common._lBody).find('*');
    ...
    page.prototype.serialize = function(elem) {
        ...
        var _push = function(index) {
            ...
            if ( jQuery.inArray(_name , _valueArr) === -1 ) {
    ...
    page.prototype.countyArea = function() {
        ...
        var $county = jQuery(common._county);
        ...
        if ( $county.length !== 0 ) {
            for ( var i = 0; i < $county.length; i ++ ) {
                ...
                $county.eq(i).data('area-text', jQuery('[name="'+_areaName+'"]').find('option').eq(0).text());

                for ( var j = 0 ; j < _area.length ; j ++ ) {
                    if ( jQuery.inArray(_area[j].county , _filter) === -1 ) {
    ...
    //
    jQuery(common._lBody).on('change', common._filter._element, function(e){
        var $self = jQuery(this),
